...
public abstract class CompanyDomainForKata
{
    ...
    {
        final Order fredOrder = new Order();
        ...
        Interval.oneTo(3).each(i -> fredOrder.addLineItem(new LineItem("saucer", 1.0)));

        fredOrder.addLineItem(new LineItem("chair", 12.50));
        fredOrder.addLineItem(new LineItem("table", 1.0));

        final Customer fred = new Customer("Fred", "London");
        fred.addOrder(fredOrder);
        this.company.addCustomer(fred);

        final Order maryOrder = new Order();
        ...
        Interval.oneTo(4).each(i -> maryOrder.addLineItem(new LineItem("cup", 1.5)));

        // TODO 7: Add 4 saucers at 1.50 each to the order
        Interval.oneTo(4).each(i ->maryOrder.addLineItem(new LineItem("saucer", 1.5)));

        maryOrder.addLineItem(new LineItem("sofa", 120.0));
        maryOrder.addLineItem(new LineItem("dog", 75.0));

        final Customer mary = new Customer("Mary", "Liphook");
        mary.addOrder(maryOrder);
        this.company.addCustomer(mary);

        final Order billOrder1 = new Order();
